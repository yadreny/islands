# hook_install.ps1
# Installs Git hooks into REPO_ROOT\.githooks with the requested boxed logging format (UTF-8 safe).
# - Per-file limit: >95 MB -> "SKIPPED (> 95 MB)" box (only if present)
# - Total limit: >150 MB -> ONE box with "TOTAL STAGED SIZE EXCEEDED ...", then INCLUDED and EXCLUDED sections
# - Report saved to .git\also_filtered_report.txt (UTF-8 no BOM) and shown by pre-push (no extra header)
# - Sizes from INDEX blobs; UTF-8 I/O; core.quotepath=false for proper filenames.
...
if (-not $repoRoot) { Die "Not a Git repository. Run from within a repo (where .git exists)." }
...
# =====================================================================
# pre-commit.ps1 (main logic; UTF-8; boxed output)
# =====================================================================
$preCommitPs1 = @'
Param(
    [int]$MaxFileMB   = 95,
...
# Box A: SKIPPED (>95 MB) — ONLY if any
...
# Box B: TOTAL (>150 MB) — ONE box with INCLUDED + EXCLUDED (ONLY if exceeded)
...
# =====================================================================
# pre-push.ps1  (prints and clears the last report; UTF-8; no extra header)
# =====================================================================
$prePushPs1 = @'
$ErrorActionPreference = "Stop"
try {
...
# =====================================================================
# sh shims (extensionless). No $(dirname); path is repo-root-relative.
# =====================================================================
$shimPreCommit = @'
#!/bin/sh
exec powershell -NoProfile -ExecutionPolicy Bypass -File .githooks/pre-commit.ps1
'@.Trim() + "`n"

$shimPrePush = @'
#!/bin/sh
exec powershell -NoProfile -ExecutionPolicy Bypass -File .githooks/pre-push.ps1
'@.Trim() + "`n"

# =====================================================================
# Write files into $hooksDir
# =====================================================================
# PS1: UTF-8 (no BOM)
$utf8NoBom = New-Object System.Text.UTF8Encoding($false)
[IO.File]::WriteAllText("$hooksDir\pre-commit.ps1", $preCommitPs1, $utf8NoBom)
[IO.File]::WriteAllText("$hooksDir\pre-push.ps1",   $prePushPs1,   $utf8NoBom)

# shims: ASCII with LF endings
[IO.File]::WriteAllText("$hooksDir\pre-commit", $shimPreCommit, (New-Object System.Text.ASCIIEncoding))
[IO.File]::WriteAllText("$hooksDir\pre-push",   $shimPrePush,   (New-Object System.Text.ASCIIEncoding))

# Set core.hooksPath for THIS repo
& git -C "$repoRoot" config core.hooksPath .githooks | Out-Null

# Best-effort chmod (harmless on Windows)
try { & bash -lc "chmod +x `"$hooksDir/pre-commit`" `"$hooksDir/pre-push`"" 2>$null | Out-Null } catch {}

Write-Host "OK. Hooks installed to: $hooksDir" -ForegroundColor Green
Write-Host (" - Per-file limit:  {0} MB" -f $MaxFileMB) -ForegroundColor DarkGreen
Write-Host (" - Total size limit: {0} MB" -f $MaxCommitMB) -ForegroundColor DarkGreen
Write-Host "UTF-8 for Git I/O enabled in hooks (Cyrillic-safe)."
